...
cluster1_instances = [{micro_instances_ips}]
cluster2_instances = [{large_instances_ips}]

@app.get("/cluster1")
def loadbalance_cluster1():
    selected_instance = random.choice(cluster1_instances)
    response = requests.get(f"http://{selected_instance}:8000/cluster1")
    return response.json()

@app.get("/cluster2")
def loadbalance_cluster2():
    selected_instance = random.choice(cluster2_instances)
    response = requests.get(f"http://{selected_instance}:8000/cluster2")
    return response.json()
...
sudo apt-get update && sudo apt-get upgrade -y
sudo apt-get install python3 python3-pip -y
sudo pip3 install fastapi uvicorn requests
echo '{LOAD_BALANCER_PY_CONTENT}' > load_balancer.py
...
def launch_instances(instance_type, count, security_group_id, user_data):
...
def main():
    ...
    micro_instances = launch_instances(INSTANCE_TYPE_T2_MICRO, 1, security_group_id, USER_DATA) #remplacer le 4 par 1 pour 4 instances 
    large_instances = launch_instances(INSTANCE_TYPE_T2_LARGE, 1, security_group_id, USER_DATA) #remplacer le 4 par 1 pour 4 instances 

    # Attendre que les instances soient dispo
    time.sleep(30)  

    print("Lancement de l'instance Load Balancer...")
    load_balancer_instance = launch_instances(INSTANCE_TYPE_T2_MICRO, 1, security_group_id, LOAD_BALANCER_USER_DATA)

    print("Instances FastAPI (t2.micro) créées : ", micro_instances)
    print("Instances FastAPI (t2.large) créées : ", large_instances)
    print("Instance Load Balancer créée : ", load_balancer_instance)
